...
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Reliability Engineering Applications}
...
The question is always: *given that a unit has survived this long, how likely
is it to fail in the next instant?* That makes hazard-based modeling a natural
fit for reliability work.

This vignette walks through five case studies that show how `dfr.dist` handles
real reliability engineering problems:

1. **Capacitor lifetime analysis** — Fit competing models to censored test data
2. **B-life calculations** — Compute B10, B50, B90 warranty metrics
3. **Warranty analysis** — Predict field failure rates from accelerated testing
4. **Maintenance scheduling** — Find optimal intervals for aging systems
5. **Competing failure modes** — Decompose additive hazards from multiple mechanisms
...
A manufacturer tests 50 ceramic capacitors to characterize their failure behavior.
The test runs for 1000 hours, after which surviving units are treated as censored.
...
```{r, fig.alt="Hazard comparison showing exponential (flat line) vs Weibull (increasing curve) for capacitors."}
...
In reliability engineering, **B-life** metrics indicate when a certain percentage
...
```{r, fig.alt="Survival curve with horizontal lines showing B10, B50, and B90 life points."}
...
```{r, fig.alt="Gompertz hazard curve showing exponentially increasing failure rate with proposed maintenance interval."}
...
```{r, fig.alt="Hazard decomposition showing electronic and mechanical failure modes combining to bathtub shape."}
...
```{r, fig.alt="Cox-Snell residuals Q-Q plot for Weibull model showing good fit."}
# Check Weibull fit for capacitor data
qqplot_residuals(weib_fit, df)
```

Points following the diagonal indicate good fit. Systematic departures suggest
...
|--------|----------|---------|
| Reliability R(t) | `surv()` | Probability of survival to time t |
| Hazard h(t) | `hazard()` | Instantaneous failure rate |
| MTTF | `inv_cdf()(0.632)` | Mean time to failure (for Weibull ≈ scale) |
| B-life | `inv_cdf()` | Time for given failure fraction |
| Failure rate | `1 - surv()(t)` | Cumulative failure proportion |

## Best Practices

1. **Always check censoring**: Right-censored data is common in reliability testing
2. **Compare models**: Use AIC/BIC to select between distributions
3. **Validate with residuals**: Cox-Snell Q-Q plots reveal misfit
4. **Consider physics**: Choose distributions that match failure mechanisms
   - Wear-out → Weibull (shape > 1)
   - Random → Exponential
   - Aging → Gompertz
5. **Account for acceleration**: Lab tests often use accelerated conditions
