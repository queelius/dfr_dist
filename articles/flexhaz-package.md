...
#>     rep(par[[1]], length(t))
#> }
#> <bytecode: 0x55578ed489f8>
#> <environment: 0x55578ed4cb48>
...
$`\hat\lambda = n / \sum t_i`$ to five decimal places. Both estimate
$`\lambda \approx 0.071`$, which is consistent with the true value of
0.08 given the sampling variability at $`n = 50`$.
...
data is $`\hat\lambda = d / \sum t_i`$ where $`d`$ is the number of
...
periodic-inspection studies: you check a device at time $`t`$ and
discover it has already failed.
...
estimate $`\hat\lambda \approx 0.072`$ is higher than a
...
The estimate $`\hat\lambda = 0.05`$ equals $`3 / 60 = d / \sum t_i`$,
