...
**Required (Imports):**
- `stats`, `numDeriv` - numerical methods
- `algebraic.dist` - distribution interface generics
- `likelihood.model` - likelihood model interface
- `generics` - generic function infrastructure

**Optional (Suggests):**
- `testthat`, `knitr`, `rmarkdown` - testing and documentation
...
- `hess_fn`: Optional Hessian function (second derivatives of log-likelihood)
...
All failure rate functions must satisfy:
1. **Non-negativity**: h(t, ...) ≥ 0 for all t
2. **Infinite cumulative hazard**: lim(t→∞) H(t) = ∞, where H(t) = ∫₀ᵗ h(u) du

These are not enforced in code but are required for mathematical correctness.

### Closure-Returning Pattern

All distribution methods follow a two-step pattern - understanding this is critical for working with the package:
...
This enables flexible parameter handling and late binding of parameters (useful for MLE).
...
| `cum_haz()` | H(t, par, ...) | ∫₀ᵗ h(u) du (numerical) |
| `surv()` | S(t, par, ...) | exp(-H(t)) |
| `cdf()` | F(t, par, ...) | 1 - S(t) |
| `density()` | f(t, par, ...) | h(t) × S(t) |
| `inv_cdf()` | t given F(t) | Uses `uniroot()` |
...
The `dfr_dist` class implements `likelihood_model` from the likelihood.model package:

| Method | Returns | Usage |
|--------|---------|-------|
| `loglik()` | Log-likelihood function | `ll <- loglik(dist); ll(df, par)` |
| `score()` | Score function (gradient) | `score_fn` → `numDeriv::grad` |
| `hess_loglik()` | Hessian of log-likelihood | `hess_fn` → `numDeriv::hessian` |
| `fit()` | MLE solver | `solver <- fit(dist); result <- solver(df, par)` → `fisher_mle` |
| `assumptions()` | Model assumptions | Returns character vector of assumptions |
...
Data frames passed to likelihood methods use:
- `t` column (or `ob_col`): Observation times
- `delta` column (or `delta_col`): Event indicators
  - `1` = exact observation (failure)
  - `0` = right-censored (survived past time t)
  - `-1` = left-censored (failed before time t)

Log-likelihood contributions:
- Exact (δ=1): log L_i = log h(t_i) - H(t_i)
- Right-censored (δ=0): log L_i = -H(t_i)
- Left-censored (δ=-1): log L_i = log(1 - exp(-H(t_i)))

### Numerical Methods

**Cumulative hazard** uses `stats::integrate()` with defaults:
- subdivisions = 1000L, abs_tol = 1e-3 (overridable)

**Sampling** uses inverse CDF method:
1. Generate u ~ Uniform(0, 1)
2. Return inv_cdf(u, par) via `uniroot()`
...
The `dfr_dist` object implements the `likelihood_model` interface and has its own `fit()` method:
```r
# Create distribution
...
- **likelihood.model**: Defines likelihood interface (loglik, score, hess_loglik) and fisher_mle class
- **numerical.mle**: Numerical optimization methods (optional, for advanced use)
- **md.tools**: Masked data utilities (Boolean matrix encoding/decoding)

## Helper Distribution Constructors

The package provides ready-to-use constructors for classic survival distributions:
...
Each provides analytical `rate`, `cum_haz_rate`, and `score_fn`. Exponential and Weibull also include analytical `hess_fn`:
...
- `R/distributions.R`: Helper constructors (dfr_exponential, dfr_weibull, etc.)
...
1. **Parameter handling**: Always use `params(x, par)` to handle NA/NULL correctly
2. **Integration warnings**: Cumulative hazard uses numerical integration - check for convergence
3. **Sampling efficiency**: Inverse CDF sampling uses uniroot - can be slow for complex rate functions
4. **Closure pattern**: Remember methods return functions, not values directly
5. **Derivative functions**: `score_fn(df, par, ...)` must return a numeric vector; `hess_fn(df, par, ...)` must return a matrix
